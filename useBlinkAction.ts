...
/**
 * Custom handlers for Blink / Solana Actions so the AI chat can drive
 * interactions handsfree instead of relying on the default Dialect renderer.
 */
export function useBlinkAction() {
  ...
  const [selectedAction, setSelectedAction] = useState<BlinkActionLink | null>(
    null
  );
  ...
  const loadMetadata = useCallback(async (blinkUrl: string) => {
    ...
    try {
      const res = await fetch(
        `/api/blinks/action?url=${encodeURIComponent(blinkUrl)}`
      );
      const json = await res.json();
      if (!res.ok || !json.success) {
        throw new Error(json.error || 'Failed to load blink metadata');
      }
  ...
  const executeAction = useCallback(
    async (blinkUrl: string, action?: BlinkActionLink | null) => {
      ...
      try {
        ...
        if (!account) {
          throw new Error('Unable to resolve wallet account address');
        }
        ...
        const res = await fetch('/api/blinks/action', {
          ...
          body: JSON.stringify({
            url: blinkUrl,
            account,
            actionHref: actionToUse?.href || '',
          }),
        });
        const json = await res.json();
        if (!res.ok || !json.success) {
          throw new Error(json.error || 'Failed to build blink transaction');
        }
        ...
        if (!payload.transaction) {
          // Some actions return a message-only response
        ...
        if (!sendRes.ok) {
          throw new Error(sendJson.message || 'Failed to send blink transaction');
        }
        ...
        setResult({
          ...
          message: 'Blink action signed and sent',
        });
        setState('success');
        return { signature };
      } catch (err) {
        const message =
          err instanceof Error ? err.message : 'Blink execution failed';
        setError(message);
        setState('error');
        throw err;
      }
  ...
  /**
   * Handsfree path: load metadata then immediately execute the default action.
   */
